...
JsKoansReporter.prototype.reportRunnerResults = function(runner) {
  ...
  if (this.failedSpecs === 0 && !hasLoadingError) {
    status = 'passed';
  ...
  var progress = this.createDom('div', {}, 
      ...
      this.createDom('div', { className: 'completion' }, 
        ...
        this.createDom('div', { className: 'suite failed', display: hasLoadingError ? 'block' : 'none' }, 'An error occurred when loading one or more files. Check your console for details.')
      );
...
var hasLoadingError;
var loadingErrorHandler = function() {
  hasLoadingError = true;
};
window.addEventListener('error', loadingErrorHandler);
window.addEventListener('DOMContentLoaded', function() {
  window.removeEventListener('error', loadingErrorHandler);
});
